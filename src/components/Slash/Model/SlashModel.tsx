...
import { connectedProvidersAtom, llmAtom, modelsListAtom, selectedModelAtom } from "../../../state/atoms";
...
const SlashModelDialog = () => {
  ...
  const options = availableModels.map(val => {
    return { name: val.name, value: val, description: val.providerInfo?.name || "" }
  })
  ...
  const handleSubmit = async (comboxboxValue: SelectOption | undefined) => {
    try {
      ...
      const createModel = pkg[`create${providerName[0]?.toUpperCase() + providerName.slice(1)}`]
  ...
  return <DialogRoot>
    ...
    <Combobox onSubmit={handleSubmit} showDescription placeholder="Model name" options={options} />
  </DialogRoot>
}
